@extends('layout')

@section('page')
    <!-- 
        headline section
      -->
    <div class="container-fluid" id="head-section">
        <div class="row headline-row">
            <h1 class="text-center headliner"></h1>
        </div>

        <!-- 
            hidden in mobile and tablet screens
         -->
        <div class="row information">
            <h1 class="text-center headline">Skriv din perfekte ansøgning og CV</h1>
            <h4 class="text-center help">Vi hjælper med Strukturering, korrekt påfylde på din ansøgning og CV</h4>
            <h4 class="text-center help">Med henblik på din personlighed og kvaliteter</h4>
            <h4 class="text-center">vi gemmer dine ansøgninger så du altid ved hvor du har dem</h4>
            <h4 class="text-center join-help"><a href="/join" class="signup-btn">Tilmeld</a> dig nu og og gør dig klar på drømmejobbet</h4>
        </div>
    </div>
@stop